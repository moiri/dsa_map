...
function Map(mapId, cache) {
     ...
     * 
     * @param int dx: x coordinate in pixel of the left border of the image
     *                (real pixel read from html) (optional, default: last position)
     * @param int dy: y coordinate in pixel of the top border of the image
     *                (real pixel read from html) (optional, default: last position)
     * @param bool drawInBuffer: if false, imgArray is not drawn in buffer
    ...
    this.drawImageToCanvas = function (dx, dy, drawInBuffer) {
        ...
        dh = me.size.dh * me.size.zoom;	// me.imaged.drawcalculate height of rendered img
                    ...
                    for (var drawOrderElem in me.images.img[drawOrderMode]) {
                        if (me.images.img[drawOrderMode].hasOwnProperty(drawOrderElem)) {
                            ...
                            for (var id in me.images.img[drawOrderMode][drawOrderElem]) {
                                if (me.images.img[drawOrderMode][drawOrderElem]
                                .hasOwnProperty(id) &&
                                (me.images.draw[id] ||
                                    (drawOrderMode === 'drawOrderMode-0')) &&
                                (me.images.img[drawOrderMode][drawOrderElem][id]
                                    !== null)) {
        ...
        me.context.drawImage(me.buffer, 0, 0, me.size.iw, me.size.ih,
            proj.x, proj.y, dw, dh);
    ...
    /**
     * prepare coordinates to be able to draw the map on the canvas element
     * (calculate projection aspects, etc)
     */
     ...
     * @param bool draw: if true, loaded image will be drawn, else it will be ignored
     *                   (-> during the next draw step deleted on canvas) 
     *                   (optionla, default = true)
     */
     ...
     * 
     * @param array imgsDb: array containing arrays with img information
     *                      [id, table, picturePath]
