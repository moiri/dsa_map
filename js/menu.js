...
function Menu() {
    ...
    me.binder.toggleMenu.clickCb = function () {
        alert('no toggleMenu click event binded, use'
            + ' "setEventBinderToggleMenu" to define the event');
    };
    ...
    this.bindEye = function (id, eyeAttr, midAttr) {
        ...
        $('#button-' + id).bind('click', function () {
            ...
            if ($(eye).hasClass('close')) {
                ...
                $('#map-canvas').animate({
                    width: $('#map-canvas').width() - me.width + me.eyeWidth + delta
                        + 'px',
                }, 'fast', 'swing');
 ...
 * @param object cache: reference to cache object, containing the stored images and
 *                      the active elements array
 */
function MainMenu(destId, cache) {
    ...
    me.binder.activeElement.clickCb = function () {
        alert('no activeElement click event binded, use'
            + ' "setEventBinderActiveElement" to define the event');
    };
    me.binder.clearActiveElement = [];
    me.binder.clearActiveElement.clickCb = function () {
        alert('no clearActiveElement click event binded, use'
            + ' "setEventBinderClearActiveElement" to define the event');
    };
    me.binder.clearActiveElements = [];
    me.binder.clearActiveElements.clickCb = function () {
        alert('no clearActiveElements click event binded, use'
            + ' "setEventBinderClearActiveElements" to define the event');
    };
    me.binder.drawElement = [];
    me.binder.drawElement.clickCb = function () {
        alert('no drawElement click event binded, use'
            + ' "setEventBinderDrawElement" to define the event');
    };
    me.binder.mode = [];
    me.binder.mode.clickCb = function () {
        alert('no mode click event binded, use'
            + ' "setEventBinderMode" to define the event');
    };
    me.binder.showDrawElementInfo = [];
    me.binder.showDrawElementInfo.clickCb = function () {
        alert('no showDrawElementInfo click event binded, use'
            + ' "setEventBinderShowDrawElementInfo" to define the event');
    };
    ...
    this.drawMenuCb = function (data, cb) {
        ...
        iteration = function (menu, destId, modeId, modeIdIt, lvl) {
            ...
            $.each(menu, function(key, val) {
                ...
                $('html > head > style').append(" ." + imgCssClass
                    + " { background: url('img/"
                    + val.iconPath + "') no-repeat scroll 50% top; }\n");
                $('html > head > style').append(" ." + imgCssClass
                    + "." + me.cssActive + " { background: url('img/"
                    + val.iconPath.replace(/\./g, "_sel.")
                    + "') no-repeat scroll 50% top; }\n");
                //$('html > head > style').append(" ." + imgCssClass
                //    + ":hover { background: url('img/"
                //    + val.iconPath.replace(/\./g, "_hov.")
                //    + "') no-repeat scroll 50% top; }\n");
                //$('html > head > style').append(" ." + imgCssClass
                //    + "." + me.cssActive + ":hover { background: url('img/"
                //    + val.iconPath.replace(/\./g, "_hov_sel.")
                //    + "') no-repeat scroll 50% top; }\n");
        ...
        .append(
            '<input id="menu-main-search-form" class="searchForm init" type="text" />'
        );
        $('#' + me.destId + ' > div.contentBox')
        .append('<div id="menu-main-content" class="content"></div>');
        ...
        $('[id^="mode-"]').bind('click', function () {
            ...
            while(myId = $('#tabCont-' + idElem[idElem.length - 1])
            .children('[id^="mode-"]:first').attr('id')) {
                idElem = myId.split('-');
                idElem.shift(); // remove descripter (keep only ids)
            }
    ...
    this.drawFreeContent = function () {
        ...
        if (me.activeElems.counter > 0) {
            $(me.selectContent)
            .append('<div id="free-category-active" class="categoryTitle"></div>');
            $('#free-category-active').append('<div id="activeElementsClear"'
                + ' class="clearElement" title="Alle Elemente deaktivieren"></div>')
            $('#free-category-active').append('<div class="categoryEye open"></div>');
            $('#free-category-active').append('<div class="categoryTitleText">'
                + 'Aktive Elemente</div>');
            $(me.selectContent).append('<div id="free-category-active-entries"'
                + ' class="category"></div>');
            ...
            drawActiveElements = function (mode, selector) {
                ...
                for (id in mode) {
                    if (mode.hasOwnProperty(id) && (mode[id].counter > 0)) {
                        $(selector).append('<div id="activeMode-' + id
                            + '" class="activeMode"></div>');
                        $('#activeMode-' + id).append('<div id="activeMode-' + id
                            + '-title" class="activeModeTitle">' + mode[id].name
                            + '</div>');
                        if (mode[id].elements !== undefined) {
                            // write out entries
                            $('#activeMode-' + id).append('<div id="activeMode-' + id
                                + '-entries" class="activeModeEntries"></div>');
                            ...
                            for (elemId in mode[id].elements) {
                                if (mode[id].elements.hasOwnProperty(elemId)) {
                                    ...
                                    .append('<div id="activeElementClear-' + id + '-'
                                        + elemId +'" class="clearElement"></div>')
                                    .append('<div id="activeElement-' + id + '-'
                                        + elemId +'" class="activeElement">'
                                        + mode[id].elements[elemId] + '</div>');
                                }
                            }
                        }
                        else {
    ...
    this.drawListContent = function (data) {
        ...
        $(me.selectContent).html('');
        lvl = 1;
        if ((activeMode = me.activeElems.mode[data.main.modeIdTree[lvl]])
                !== undefined) {
            while (activeMode.elements === undefined) {
                lvl++;
                if ((activeMode = activeMode.mode[data.main.modeIdTree[lvl]])
                        === undefined) {
                    break;
                }
            }
        ...
        $.each(data, function (key, val) {
            ...
            if ((key !== 'main') && (mode !== 'search')) {
                $(me.selectContent).append('<div id="category-' + val.id
                    + '" class="categoryTitle"></div>');
                selectCategory = '#category-' + val.id;
                $(selectCategory).append('<div class="categoryEye"></div>');
                $(selectCategory).append('<div class="categoryTitleText">' + key
                    + '</div>');
                $(me.selectContent).append('<div id="category-' + val.id
                    + '-entries" class="category"></div>');
                selectCategoryEntries += ' > ' + selectCategory + '-entries';
            }
            $.each(val.entries, function (key, val) {
                var active, elem;
                active = '';
                elem = $('<div class="drawElementCont"></div>')
                .appendTo(selectCategoryEntries);
                ...
                $('<div id="drawElement-' + data.main.activeMode + '-' + val.id
                    + '" class="drawElement' + active + '">' 
                    + val.name + '</div>')
                .appendTo(elem)
                .bind('click', function () {
                ...
                if ((activeMode !== undefined) && (activeMode.counter > 0)) {
                    if ((activeMode.elements[val.id] !== undefined) &&
                        (activeMode.elements[val.id])) {
     ...
     * (adds and removes classes to show if an active element is in the corresponding tab
     * or not)
     * 
     * @param string id: id of the element to be removed or added to the active elements
     * @param string name: name of the element to be removed or added to the active
     *                     elements
...
function InfoMenu(destId) {
    ...
    this.drawMenu = function (cb) {
        ...
        $('#map-canvas').width($('#map-canvas').width() - me.width - me.eyeWidth);
        $('#' + me.destId).append('<div id="' + me.destId
    ...
    this.drawInitContent = function () {
        $(me.selectContent).html('');
        $(me.selectContent)
        .append('<div class="noInfoActive">kein Element ausgew&auml;hlt</div>');
    };
}
